...
export default function BuyerAlertNotification({ navigation }) {
  ...
  // --- LIVE DATA FETCHING ---
  // useFocusEffect ensures the inbox refreshes every time the user opens this page
  useFocusEffect(
    useCallback(() => {
      fetchData();
    }, [activeTab])
  ...
  const fetchData = async () => {
    try {
      if (activeTab === 'notifications') {
        // Fetch personal notifications (Order updates, etc.)
        const response = await apiClient.get('/notifications');
        if (response.data.success) {
          setNotifications(response.data.data);
        }
      } else {
        // Fetch public admin alerts
  ...
  // --- RENDER NOTIFICATION (Personal Order Updates) ---
  const renderNotification = ({ item }) => (
    ...
    >
      <View style={[styles.notifIconCircle, { backgroundColor: item.isRead ? '#f0f0f0' : K_GREEN + '20' }]}>
        <Ionicons 
            name={item.type === 'NewOrder' ? 'cart' : item.type === 'AdminAlert' ? 'megaphone' : 'notifications'} 
            size={20} 
  ...
  // --- RENDER ALERT (Public System Broadcasts) ---
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabBar}>
        ...
        >
          <Text style={[styles.tabText, activeTab === 'alerts' && styles.activeTabText]}>System Alerts</Text>
        </TouchableOpacity>
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
                <Ionicons name="mail-open-outline" size={60} color="#eee" />
                <Text style={styles.emptyText}>Nothing in your {activeTab} yet</Text>
            </View>
          }
        />
      )}
